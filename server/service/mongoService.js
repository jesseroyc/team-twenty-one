        ...
        MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
          ...
          client.db(DB_NAME).collection(COL_NAME).find({}).toArray(function(err, result) {
            if (err) {
              ...
              let timArr = result.map(ele => new Date().getTime() - parseFloat(ele.tim)).filter(num => !Number.isNaN(num));
              ...
              let data = {tim:timArr,tmp:tmpArr,moi:moiArr,hum:humArr,pre:preArr};
              logger.info(data);
              resolve(data);
            }
          });
